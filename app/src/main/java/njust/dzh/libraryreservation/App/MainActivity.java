...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        ...
        if (clickedId == R.id.reservation) {
            handleReservation();
        } else if (clickedId == R.id.check) {
            navigateToCheck();
        } else if (clickedId == R.id.student) {
            navigateToStudent();
        } else if (clickedId == R.id.step) {
            navigateToStep();
        } else if (clickedId == R.id.exit) {
            exitApp();
        }
    }

    // 拆分具体逻辑到独立方法（可选，提升可读性）
